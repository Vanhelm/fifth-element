<template>
  <div id="intro" class="basic-1">
    <div class="container">
      <div class="row">
        <div class="card yellow">
          <div class="card-header yellow">
            <h4>15% теории <br>
              только важная информация</h4>
            <div class="card-img">
              <img src="@/assets/earth.svg" alt="">
            </div>
          </div>
          <div class="card-body yellow">
            <p class="yellow">
              Вы узнаете: <br> <br>
              1) Как проверить заказчика на законную деятельность <br> <br>
              2) Как отличить выгодный тендер от не выгодного <br> <br>
              3) Как найти надёжного поставщика <br> <br>
              4) Как выйти на производителя <br> <br>
              5) Тонкости и подводные камни госзакупок <br> <br>
            </p>
          </div>
        </div>
        <div class="card red">
          <div class="card-header red">
            <h4>85% практики <br>
              быстрый результат</h4>
            <div class="card-img">
              <img src="@/assets/fire.svg" alt="">
            </div>
          </div>
          <div class="card-body red">
            <p class="red">
              Вы сможете: <br> <br>

              1) Находить поставщиков за 5 минут <br> <br>

              2) Масштабировать действующий бизнес <br> <br>

              3) Автоматизировать процесс победы в тендерах <br> <br>

              4) Заключать безопасные сделки <br> <br>

              5) Вовремя получать оплату от заказчика
            </p>
          </div>
        </div>
        <div class="card green">
          <div class="card-header green">
            <h4>Попутный ветер<br>
              для ваших крыльев</h4>
            <div class="card-img">
              <img src="@/assets/air.svg" alt="">
            </div>
          </div>
          <div class="card-body green">
            <p class="green">
              Базовый пакет шаблонов документов дляреализации <br>
              заказа <br> <br>

              Выявление спроса со стороны крупных  заказчиков <br>

              Полгода доступа к материалам курса <br> <br>

              Обучение на настоящей заявке. <br> <br>

              Помощь для учеников, говорящих на казахском. <br>
            </p>
          </div>
        </div>
        <div class="card blue">
          <div class="card-header blue">
            <h4>Вливайся<br>
              в наше сообщество</h4>
            <div class="card-img">
              <img src="@/assets/water.svg" alt="">
            </div>
          </div>
          <div class="card-body blue">
            <p class="blue">
              Эксперты курса поделятся приёмами и фишками <br>
              о которых вам больше никто не расскажет <br> <br>

              Получай помощь от координаторов и наставников <br> <br>

              Создавай полезные знакомства общаясь с однокурсниками <br> <br>

              Получай помощь от координаторов <br>
              и наставников
            </p>
          </div>
        </div>
        <div class="second_btn"><Button></Button></div>
      </div> <!-- end of row -->
    </div> <!-- end of container -->
  </div> <!-- end of basic-1 -->
</template>

<script>
import Button from "./UI/Button";
export default {
  name: 'card',
  components: {
    Button,
  }
}
</script>

<style lang="scss" scoped>
.basic-1 {
  background-color: #262626;
  padding-bottom: 2rem;
  @media(max-width: 1140px) {
    padding-top: 0;
  }

  .container {
    width: 100%;
    margin: 0 auto;

    .row {
      display: flex;
      flex-direction: row;
      justify-content: space-around;
      flex-flow: wrap;


      .card {
        padding-bottom: 0;
        display: flex;
        flex-direction: column;
        border: 1px solid #2D2D2D;
        margin-bottom: 25px;
        transition: 0.3s;
        background: #2D2D2D;
        border-radius: 10px;
        width: 500px;
        @media(max-width: 1140px) {
          .card {
            max-width: 100%;
          }
        }

        .card-header {
          display: flex;
          padding: 27px;
          background-color: #2D2D2D;
          color: #fff;
          justify-content: space-between;
          @media(max-width: 1140px) {
            .card-header {
              width: 100%;
            }
          }

          h4 {
            display: flex;
            justify-content: space-between;
            font-family: Raleway;
            font-style: normal;
            font-weight: bold;
            font-size: 24px;
            line-height: 28px;
            @media(max-width: 1140px) {
              font-size: 18px;
              line-height: 21px;
            }
          }

          p {
            font-family: Raleway;
            font-style: normal;
            font-weight: bold;
            font-size: 16px;
            line-height: 16px;
          }

          .card-img {
            display: flex;
            position: relative;
          }
        }


        .card-body {
          background-color: #2D2D2D;
          color: #FFFFFF;
          padding: 30px 20px;
          font-size: 18px;
          height: 100%;
          @media(max-width: 1140px) {
            .card-body {
              width: 100%;
            }
          }

          p {
            color: #FFFFFF;
            text-align: left;
          }


        }

      }

      .card:hover {
        transform: scale(1.05);
        box-shadow: 0 0 40px -10px rgba(0, 0, 0, 0.25);

        .yellow {
          background: #FFED69;
          color: #262626;
        }

        .red {
          background: #FF5E48;
          color: #262626;
        }

        .green {
          background: #A7F271;
          color: #262626;
        }

        .blue {
          background: #1265F5;
          color: #262626;
        }
      }
      .second_btn {
        @media(max-width: 1140px) {
          width: 100%;
        }
      }
    }
  }
}
</style>
